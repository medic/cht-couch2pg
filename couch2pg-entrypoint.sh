...
check_if_postgres_is_ready(){
  ...
  wait_count=0
  echo "export POSTGRESQL_URL=postgres://$POSTGRES_USER_NAME:$POSTGRES_PASSWORD@$POSTGRES_DB:5432/$POSTGRES_DB_NAME"
  echo "pg_isready -q  -h $POSTGRES_DB -U $POSTGRES_USER_NAME --d $POSTGRES_DB_NAME"
